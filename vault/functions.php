 ...
 */
$CIDRAM['ParseVars'] = function (array $Needles, $Haystack) {
    if (empty($Haystack)) {
        return '';
    }
    ...
    for ($FileIndex = 0; $FileIndex < $Counts['Files']; $FileIndex++) {
        ...
        for ($FactorIndex = 0; $FactorIndex < $Counts['Factors']; $FactorIndex++) {
            ...
            while (true) {
                ...
                $RunExitCode = 0;
                if ($Category === 'Run' && !$CIDRAM['CIDRAM_sapi'] && $Signature !== '') {
...
$CIDRAM['InitialiseCache'] = function () use (&$CIDRAM) {
    ...
    $CIDRAM['Cache']->RedisDatabaseNumber = $CIDRAM['Config']['supplementary_cache_options']['redis_database_number'];
    ...
    foreach ($CIDRAM['VerificationData'][$From] as $Name => $Values) {
        ...
        if (is_array($Values) && isset($Values['Closure'], $Values['Valid domains']) && (
            ...
            $Options = [
                ...
                'Can modify trackable' => isset($Values['Can modify trackable']) ? $Values['Can modify trackable'] : true
            ];
 ...
 * @return void
 */
$CIDRAM['OtherVerification'] = function () use (&$CIDRAM) {
...
$CIDRAM['LogRotation'] = function ($Pattern) use (&$CIDRAM) {
    ...
    if ($Count > $Limit) {
        ...
        $StageRestore = isset($CIDRAM['Stage']) ? $CIDRAM['Stage'] : '';
        $CIDRAM['Stage'] = '';
...
$CIDRAM['Aux'] = function () use (&$CIDRAM) {
    ...
    foreach ($CIDRAM['AuxData'] as $Name => $Data) {
        ...
        /** Run target. */
        $Run = (string)(empty($Data['Run']['File']) ? '' : $Data['Run']['File']);
 ...
 * @return void
 */
$CIDRAM['AddProfileEntry'] = function ($Entries) use (&$CIDRAM) {
...
if (isset($CIDRAM['Vault'])) {
    ...
    if (!empty($CIDRAM['Config']['general']['events'])) {
        foreach (array_unique(explode(',', $CIDRAM['Config']['general']['events'])) as $CIDRAM['LoadThis']) {
            if (
