...
if ($CIDRAM['Protect'] && !$CIDRAM['Config']['general']['maintenance_mode'] && empty($CIDRAM['Whitelisted'])) {
    ...
    if (empty($CIDRAM['Whitelisted']) && $CIDRAM['Config']['signatures']['modules']) {
        ...
        if (!$CIDRAM['Config']['signatures']['tracking_override']) {
            $CIDRAM['Restore tracking options override'] = isset($CIDRAM['Tracking options override']) ? $CIDRAM['Tracking options override'] : '';
        }
...
if (!empty($CIDRAM['Webhooks']) || !empty($CIDRAM['Config']['Webhook']['URL'])) {
    ...
    /** Set timeout. */
    $CIDRAM['WebhookTimeout'] = empty(
        $CIDRAM['Config']['Webhook']['Timeout']
    ) ? $CIDRAM['Request']->DefaultTimeout : $CIDRAM['Config']['Webhook']['Timeout'];
...
if ($CIDRAM['BlockInfo']['SignatureCount'] > 0) {
    ...
    if (!empty($CIDRAM['Banned']) && $CIDRAM['Config']['general']['ban_override'] > 400 && (
        ...
        header('Retry-After: ' . isset($CIDRAM['RL_RetryAfter']) ? $CIDRAM['RL_RetryAfter'] : floor($CIDRAM['Config']['rate_limiting']['allowance_period'] * 3600));
        $CIDRAM['HTML'] = '';
